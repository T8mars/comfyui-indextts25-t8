...
class Tokenizer:
    """Minimal tiktoken wrapper needed by IndexTTS inference.

    Upstream subclasses Whisper's tokenizer, but the TTS path only needs
    ``encode`` and ``decode``.  Keeping this tiny local wrapper avoids pulling
    the complete openai-whisper/numba stack into ComfyUI.
    """

    def __init__(self, encoding, num_languages, language=None, task=None, **kwargs):
        self.encoding = encoding
        self.num_languages = num_languages
        self.language = language
        self.task = task
        self.special_tokens = {
            token: self.encoding.encode_single_token(token)
            for token in self.encoding.special_tokens_set
        }
    ...
    def decode(self, token_ids, **kwargs):
        token_ids = [token for token in token_ids if token < self.timestamp_begin]
