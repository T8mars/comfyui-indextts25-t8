...
当前版本基线：**ComfyUI Node 0.23.0 · Desktop 0.24.0 · Core `ee40fa7d` · Upstream Model `c39ce5ba`**。

### v0.23.0 人类可读逐句情感与时间轴交换

- 批量台词推荐使用一行一句的 `角色|台词|语言|时长系数|逐句情感`，无需手写 JSON；支持 `text:`、八维 `vector:`、`speaker`、`;strength=0~1` 与 `;random=true`。
- 原有 JSON 数组完全兼容；新增“一行一句可编辑批量脚本”输出，台词中的 `|` 和换行可安全往返。
- 示例 31 改为人类可读格式，全部 33 组 UI/API 工作流刷新到 0.23.0。
- 配套 Desktop 0.24.0 在时间轴旁提供八维含义速查，并支持 JSON/CSV 可编辑时间轴导入导出。
...
- 长文本内部每个实际合成分段都会记录文字单位数、真实音频时长和单位/秒；至少两个稳定分段后才建立中位语速基线。
...
- Linux 与 Windows 发布门禁新增针对 `services/model_store.py` 的 Pylint/Astroid Fatal 检查，可在发布前拦截能够由源码复现的 `F0002/astroid-error`。
...
- 多角色台词未显式填写语言时，正确继承对应角色音色的默认语言；同时修复逐句情感字符串 `false`、非法字幕实际时间及示例 33 角色映射。
- 模型哈希缓存增加并发保护；更新检查改为真正的 1 MiB 流式上限。
- Registry 包移除未使用训练权重，并随包提供 BigVGAN 上游第三方许可证全文。
- 停止检查只复用 ComfyUI 已加载的运行时模块，避免纯 CPU 环境因节点主动初始化 CUDA 而失败。
...
- 完整 SHA-256 校验与模型缓存指纹加入不可由普通时间戳恢复覆盖的文件变更标记，能够识别同尺寸替换。
- “释放全部模型”遇到正在生成的任务时改为延迟释放，避免下一任务重复加载第二份模型；ASR 清理也会等待当前识别结束。
- 模型缺失文件总量加 1 GiB 安全余量超过可用空间时，在联网下载前直接停止并显示所需/可用空间。
- audio.cpp 组件安装增加断点完整性、解压空间预检、事务替换、清单路径约束及 Windows 文件变更检测。
- 已保存音色包拒绝 Windows 大小写/Unicode 路径别名和重复角色，并避免刷新缓存时不断创建新目录。
...
- 新增“audio.cpp 一键组件”节点：由用户明确确认后下载官方 Windows CUDA/Vulkan/CPU 运行时和 Q8/F16/original GGUF，支持断点续传、磁盘预检与 SHA-256 校验。默认 Python 推理完全不受影响。
- audio.cpp 生成节点的路径可留空，会自动使用一键组件安装结果；手动绝对路径仍然优先。
- 示例扩充到 34 组：`33_saved_voice_library.json` 与 `34_audiocpp_one_click.json` 分别演示共享音色和一键组件。

### v0.19.0 模型下载进度与磁盘预检

- 模型加载器的“缺失时自动下载/修复完整模型”现在把扫描、下载和 SHA-256 校验映射到 ComfyUI 任务进度，不再只能观察终端下载条。
- 下载按固定清单逐文件执行；ComfyUI 显示总体进度，终端同步显示当前阶段、文件、速度和 ETA。保留 Hugging Face 断点续传，损坏文件仍强制重新下载。
- 下载前报告保守的缺失字节数与目标磁盘可用空间；低于安全下限时在写入大文件前给出明确错误。
- 自动修复结束后再次校验全部 26 个主模型与辅助文件，失败时继续报告具体缺失或哈希不匹配路径。
...
7. `IndexTTS 2.5 角色音色 · T8star-Aix`
   ...
   - 配套“已保存音色”节点可直接加载 Desktop `.t8voice.zip`，免去重复上传参考音频
...
```text
角色A|我先平静地说明情况。|ZH|1.0|text:平静、从容;strength=0.75
角色A|你为什么一直在骗我！|ZH|1.0|vector:0,0.8,0,0,0,0,0,0;strength=0.85
角色A|这一句恢复角色默认情感。|ZH|1.0
```

这是一行一句的推荐写法，无需手写 JSON。情感末尾可追加 `;strength=0~1`，向量还可追加
`;random=true`；台词正文中的分隔符 `|` 写作 `\|`，正文换行写作 `\n`。节点的第三个输出
“一行一句可编辑批量脚本”可直接复制、修改或保存。原有 JSON 完全兼容：
